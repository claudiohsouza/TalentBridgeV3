...
('Estágio em Front-end', 'Buscamos jovem com vontade de aprender e interesse em desenvolvimento web para atuar com React.', 'Estágio', 'Desenvolvimento de Software', ARRAY['HTML', 'CSS', 'JavaScript', 'React'], ARRAY['Vale Transporte', 'Vale Refeição'], 'R$ 1.500,00', 'Remoto', 'aprovado', (SELECT id FROM chefes_empresas WHERE empresa = 'TechX'), '2024-06-01', '2024-12-01', 2),
('Analista de Qualidade Júnior', 'Vaga para atuar em controle de qualidade de alimentos em indústria moderna.', 'CLT', 'Qualidade', ARRAY['Excel', 'Gestão', 'Comunicação'], ARRAY['Plano de Saúde', 'Vale Alimentação'], 'R$ 2.800,00', 'Presencial - RJ', 'aprovado', (SELECT id FROM chefes_empresas WHERE empresa = 'FoodNow'), '2024-06-10', '2024-12-10', 1),
('Estágio em Psicologia Organizacional', 'Acompanhar projetos de desenvolvimento humano e clima organizacional.', 'Estágio', 'Recursos Humanos', ARRAY['Psicologia', 'Comunicação', 'Empatia'], ARRAY['Vale Transporte', 'Bolsa Auxílio'], 'R$ 1.200,00', 'Presencial - SP', 'aprovado', (SELECT id FROM chefes_empresas WHERE empresa = 'TechX'), '2024-07-01', '2024-12-31', 1);

-- Exemplos adicionais de oportunidades úteis
INSERT INTO oportunidades (titulo, descricao, tipo, area, requisitos, beneficios, salario, local, status, empresa_id, data_inicio, data_fim, vagas_disponiveis) VALUES
('Estágio em Análise de Dados', 'Auxiliar na coleta, tratamento e análise de dados para apoiar decisões estratégicas.', 'estagio', 'analise_dados', ARRAY['Excel', 'Python', 'Power BI', 'Raciocínio lógico'], ARRAY['Vale Transporte', 'Bolsa Auxílio', 'Seguro de Vida'], 'R$ 1.200,00', 'Remoto', 'aprovado', (SELECT id FROM chefes_empresas WHERE empresa = 'TechX'), '2024-08-01', '2025-01-31', 3),
('Desenvolvedor Back-end Júnior', 'Atuar no desenvolvimento e manutenção de APIs e sistemas internos.', 'clt', 'desenvolvimento_software', ARRAY['Node.js', 'PostgreSQL', 'APIs REST', 'Git'], ARRAY['Plano de Saúde', 'Vale Refeição', 'Auxílio Home Office'], 'R$ 3.000,00', 'Híbrido - São Paulo', 'aprovado', (SELECT id FROM chefes_empresas WHERE empresa = 'TechX'), '2024-09-01', '2025-09-01', 2),
('Analista de Marketing Digital', 'Planejar e executar campanhas digitais, análise de métricas e SEO.', 'clt', 'marketing_digital', ARRAY['Google Ads', 'SEO', 'Redes Sociais', 'Analytics'], ARRAY['Vale Transporte', 'Vale Alimentação', 'Day Off Aniversário'], 'R$ 2.500,00', 'Presencial - Belo Horizonte', 'aprovado', (SELECT id FROM chefes_empresas WHERE empresa = 'FoodNow'), '2024-08-15', '2025-08-15', 1),
('Estágio em Engenharia Mecânica', 'Acompanhar projetos de manutenção e automação industrial.', 'estagio', 'engenharia_mecanica', ARRAY['AutoCAD', 'SolidWorks', 'Noções de manutenção'], ARRAY['Vale Transporte', 'Refeitório no local'], 'R$ 1.100,00', 'Presencial - Campinas', 'aprovado', (SELECT id FROM chefes_empresas WHERE empresa = 'FoodNow'), '2024-09-10', '2025-03-10', 2),
('Assistente Administrativo', 'Auxiliar nas rotinas administrativas, atendimento e organização de documentos.', 'clt', 'administracao', ARRAY['Pacote Office', 'Organização', 'Boa comunicação'], ARRAY['Vale Transporte', 'Vale Alimentação'], 'R$ 1.800,00', 'Presencial - Rio de Janeiro', 'aprovado', (SELECT id FROM chefes_empresas WHERE empresa = 'FoodNow'), '2024-08-20', '2025-02-20', 1),
('Designer Gráfico Freelancer', 'Criação de peças para redes sociais e campanhas digitais.', 'freelancer', 'design_grafico', ARRAY['Photoshop', 'Illustrator', 'Portfólio'], ARRAY['Horário Flexível'], 'A combinar', 'Remoto', 'aprovado', (SELECT id FROM chefes_empresas WHERE empresa = 'TechX'), '2024-08-05', '2024-12-31', 1),
('Estágio em Psicologia Escolar', 'Apoiar projetos de orientação e acompanhamento de alunos.', 'estagio', 'psicologia', ARRAY['Cursando Psicologia', 'Empatia', 'Boa comunicação'], ARRAY['Vale Transporte', 'Bolsa Auxílio'], 'R$ 1.000,00', 'Presencial - São Paulo', 'aprovado', (SELECT id FROM chefes_empresas WHERE empresa = 'TechX'), '2024-08-20', '2025-01-20', 1);
